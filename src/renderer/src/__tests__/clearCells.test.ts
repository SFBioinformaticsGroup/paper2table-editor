...
describe('clearCells', () => {
  it('nulls out targeted cells', () => {
    const file = makeFile(
      flatTable([
        { compound: 'aspirin', dose: '100mg', route: 'oral' },
      ])
    )
    const result = clearCells(file, 0, 0, [0], ['compound', 'dose'])
    expect((result.tables[0] as { rows: Row[] }).rows).toEqual([
      { compound: null, dose: null, route: 'oral' },
    ])
  })
  ...
  it('leaves untargeted rows untouched', () => {
    ...
    const result = clearCells(file, 0, 0, [1], ['compound'])
    expect((result.tables[0] as { rows: Row[] }).rows).toEqual([
      { compound: 'aspirin', dose: '100mg' },
  ...
  it('leaves untargeted columns untouched', () => {
    const file = makeFile(
      flatTable([
        { compound: 'aspirin', dose: '100mg', route: 'oral' },
      ])
    )
    const result = clearCells(file, 0, 0, [0], ['dose'])
    expect((result.tables[0] as { rows: Row[] }).rows).toEqual([
  ...
  it('only modifies the targeted fragment', () => {
    ...
    const result = clearCells(file, 0, 0, [0], ['compound'])
    const fragments = (result.tables[0] as { table_fragments: { rows: Row[] }[] }).table_fragments
    expect(fragments[0].rows[0]).toEqual({ compound: null })
