...
export function App() {
  ...
  const callbacks: EditorCallbacks = useMemo(
    () => ({
      ...
      clearColumn: (fileName, options) =>
        applyEdit(fileName, (f) => clearColumn(f, { ...options, editColumnsGlobally })),
      deleteColumn: (fileName, options) =>
        applyEdit(fileName, (f) => deleteColumn(f, { ...options, editColumnsGlobally })),
      duplicateColumn: (fileName, options) =>
        applyEdit(fileName, (f) => duplicateColumn(f, { ...options, editColumnsGlobally })),
      splitColumn: (fileName, options) =>
        applyEdit(fileName, (f) => splitColumn(f, { ...options, editColumnsGlobally })),
      renameColumn: (fileName, newName, options) =>
        applyEdit(fileName, (f) => renameColumn(f, newName, { ...options, editColumnsGlobally })),
      mergeColumns: (fileName, dropCol, separator, options) =>
        applyEdit(fileName, (f) => mergeColumns(f, dropCol, separator, { ...options, editColumnsGlobally })),
      addColumn: (fileName, columnName, options) =>
        applyEdit(fileName, (f) => addColumn(f, columnName, { ...options, editColumnsGlobally })),
      transferColumnValues: (fileName, destColName, options) =>
        applyEdit(fileName, (f) => transferColumnValues(f, destColName, { ...options, editColumnsGlobally })),
